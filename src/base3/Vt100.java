...
public class Vt100 {
    ...
    Vt100(SipPhone sipPhone) {
        sipPhoneCla = sipPhone;
    }
    ...
    int chkStrPos(int stInx,int endInx,String str,int pos){
        ...
        if(pos==0){//first
            ...
            for(i=0;i<strLen;i++){
                inx = inx & 255;
                if(cmpbuf[inx--] != bytes[strLen - i - 1])
                    return -1;
            }
            return inx+1;
        }
    ...
    String ncmpStar(String starStr,String str,int maxLen) {
        byte[] bytes;
        String[] strA= str.split(starStr);
        if(strA.length<=1){
            return null;
        }
        int len=strA.length;
        int stInx = cmpbuf_inx;
        int endInx=cmpbuf_inx-maxLen;
        int[] posA=new int[len];
        String[] paraA=new String[len];
        endInx=endInx&255;
        for(int i=0;i<len;i++){
            String str1=strA[len-1-i];
            posA[i]=chkStrPos(stInx,endInx,str1,i);
            if(posA[i]<0)
                return null;
            stInx=posA[i]-1;
        }
        for(int i=0;i<len-1;i++){
            int endPos=posA[i];
            int stPos=posA[i+1]+strA[len-2-i].length();
            paraA[i]="";
            for(;;){
                stPos=stPos&255;
                if(stPos==endPos)
                    break;
                paraA[i]+=(char)cmpbuf[stPos++];
            }
        }
        String paraStr="";
        for(int i=0;i<len-1;i++){
